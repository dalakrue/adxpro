"""Main Home tab entry with lazy implementation loading."""

import streamlit as st


def _render_top_copy_button():
    """Top-of-Home copy button.

    Copies Home + Doo Prime Analysis/Finder/modeling state, but intentionally
    excludes detailed per-position entry rows so the payload stays compact.
    """
    try:
        from .legacy import implementation as impl
        build = getattr(impl, "_build_home_copy_payload", None)
        copy_box = getattr(impl, "_copy_home_data_box", None)
        if not callable(build) or not callable(copy_box):
            return
        with st.container():
            st.markdown("### 📋 Home Master Copy")
            st.caption("Copies all Home-related data plus Doo Prime Analysis/Finder summaries. Detailed position-entry rows are excluded.")
            c1, c2 = st.columns([1, 2])
            with c1:
                if st.button("📋 Build + Copy Home + Doo Analysis", key="home_top_master_copy_build", use_container_width=True):
                    st.session_state.home_copy_export_text = build()
                    st.session_state.home_copy_export_built_at = "top"
            text = st.session_state.get("home_copy_export_text", "")
            if text:
                copy_box(text=text)
    except Exception as exc:
        with st.expander("Show top copy error", expanded=False):
            st.exception(exc)


def show():
    try:
        _render_top_copy_button()
        from .legacy.implementation import show as _show
        return _show()
    except Exception as exc:
        st.error("Home implementation could not be loaded safely.")
        with st.expander("Show Home import/runtime error", expanded=True):
            st.exception(exc)
